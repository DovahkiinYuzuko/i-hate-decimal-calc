...
> `ihd` が生成した Lean 4 証明コードは、GitHub Actions CI 上で Lean 4 / Mathlib4 環境を用いて実際にコンパイル検証を行っています。多項式の因数分解（`by ring`）、導関数の計算（`by simp`）、Wu法による幾何自動証明などが Lean 4 カーネルを通り、`#print axioms` において `sorry`（`sorryAx`）が使われていないことが確認されています。
...
> Lean 4 proof code generated by `ihd` is compiled and checked in GitHub Actions CI using the Lean 4 and Mathlib4 environment. Polynomial factorizations (`by ring`), derivative evaluations (`by simp`), and Wu's method geometric proofs successfully pass Lean 4 kernel checks without relying on `sorry` (`sorryAx`), as verified via `#print axioms`.
