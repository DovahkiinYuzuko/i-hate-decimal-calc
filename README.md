...
[![Lean 4 Verification](https://github.com/DovahkiinYuzuko/i-hate-decimal-calc/actions/workflows/lean-verify.yml/badge.svg)](https://github.com/DovahkiinYuzuko/i-hate-decimal-calc/actions/workflows/lean-verify.yml)
...
> `ihd` が生成した Lean 4 証明コードの一部は、GitHub Actions CI 上の Lean 4 / Mathlib4 環境で継続的にコンパイル・検証されています。現在、多項式の因数分解（`by ring`）、導関数計算（`by simp`）、Wu法による幾何自動証明などの代表ケースが通過しており、`#print axioms` により少なくとも `sorry`（`sorryAx`）を含まないことを確認しています。
...
> Lean 4 proof code generated by `ihd` is compiled and checked in GitHub Actions CI using a Lean 4 + Mathlib4 environment. Polynomial factorizations (`by ring`), derivative evaluations (`by simp`), and Wu's method geometric proofs have successfully passed Lean verification without relying on `sorry` (`sorryAx`), as confirmed via `#print axioms`.
